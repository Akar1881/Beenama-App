...
export default function AnimeScreen() {
  ...
  const loadAnime = async () => {
    setLoading(true);
    try {
      const response = await fetchAnime(page, filters);
      setAnime(page === 1 ? response : [...anime, ...response]);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filterRow}>
        <Text style={styles.title}>Explore Anime</Text>
      ...
      <FilterModal 
        ...
        type="tv" // Anime uses TV genres mostly
      />
    </View>
  );
}
