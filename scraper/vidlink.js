...
function makeRequest(url, options = {}) {
    ...
    // Improved proxy check - only use proxy for specific domains that require it
    // and skip for data that might be already proxied or direct
    const needsProxy = isWeb && (
        url.includes('vidlink.pro') || 
        url.includes('enc-dec.app') ||
        (url.includes('vodvidl.site') && !url.includes('corsproxy.io'))
    );
    
    const proxyUrl = needsProxy ? `https://corsproxy.io/?${encodeURIComponent(url)}` : url;
    ...
    .then(response => {
        if (!response.ok) {
            // If proxy failed with 403, try direct as fallback for vodvidl.site
            if (response.status === 403 && needsProxy && url.includes('vodvidl.site')) {
                console.log(`[Vidlink] Proxy 403, retrying direct: ${url.substring(0, 50)}...`);
                return fetch(url, {
                    method: options.method || 'GET',
                    headers: defaultHeaders,
                    ...options
...
function fetchAndParseM3U8(playlistUrl, mediaInfo, customHeaders = null) {
    console.log(`[Vidlink] Fetching M3U8 playlist: ${playlistUrl.substring(0, 80)}...`);
    
    const headers = customHeaders || WORKING_HEADERS;

    return makeRequest(playlistUrl, { headers })
...
function processVidlinkResponse(data, mediaInfo) {
    ...
    try {
        ...
        rawCaptions.forEach(cap => {
            if (cap.url) {
                subtitles.push({
                    id: cap.id || cap.url,
                    url: cap.url,
                    label: cap.language || cap.name || 'Unknown',
        ...
        if (data.stream && (data.stream.qualities || data.stream.playlist)) {
            console.log(`[Vidlink] Processing stream object`);

            // Extract headers from playlist URL if present
            let streamHeaders = WORKING_HEADERS;
            const playlistUrl = data.stream.playlist || '';
            
            if (playlistUrl.includes('?headers=')) {
                try {
                    ...
                    if (headersParam) {
                        ...
                        // Ensure keys are correct for fetch headers (Referer/Origin capitalized)
                        if (streamHeaders.referer) {
                            streamHeaders.Referer = streamHeaders.referer;
                            delete streamHeaders.referer;
                        }
                        if (streamHeaders.origin) {
                            streamHeaders.Origin = streamHeaders.origin;
                            delete streamHeaders.origin;
                        }
                        console.log('[Vidlink] Extracted custom headers from URL');
                    }
                } catch (e) {
                    console.error('[Vidlink] Error parsing headers from URL:', e);
                }
            }

            // Clean playlist URL (strip query params for the actual stream source)
            const cleanPlaylistUrl = playlistUrl.split('?')[0];
...
export function getStreams(tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null) {
    ...
    .then(tmdbInfo => {
        ...
        .then(encryptedId => {
            ...
            .then(data => {
                ...
                const { streams, subtitles } = processVidlinkResponse(data, mediaInfo);
                ...
                if (playlistStreams.length > 0) {
                    ...
                    const playlistPromises = playlistStreams.map(ps => 
                        fetchAndParseM3U8(ps.url, ps.mediaInfo, ps.headers)
                    );
                    ...
                    return Promise.all(playlistPromises)
                        .then(parsedStreamArrays => {
                            ...
                            return { streams: allStreams, subtitles };
                        });
                } else {
                    ...
                    return { streams: directStreams, subtitles };
                }
            });
        });
    })
