...
async function makeRequest(url, options = {}) {
  // Safe check for Web platform without requiring 'react-native' module
  const isWeb = typeof window !== 'undefined' && (
    window.location || 
    (typeof navigator !== 'undefined' && navigator.product === 'ReactNative')
  );
  
  // Use CORS proxy only for Web to avoid CORS issues with third-party APIs
  const needsProxy = isWeb && (
    ...
    url.includes('frostveil88.live') ||
    url.includes('.m3u8')
  );
  
  const finalUrl = needsProxy ? `https://corsproxy.io/?${encodeURIComponent(url)}` : url;
  ...
  try {
    const response = await fetch(finalUrl, __spreadValues({
...
function fetchAndParseM3U8(playlistUrl, mediaInfo) {
  return __async(this, null, function* () {
    ...
    try {
      // Use makeRequest which handles CORS proxy for web and keeps headers
      const response = yield makeRequest(playlistUrl, { 
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36",
          "Accept": "*/*",
          "Referer": "https://vidlink.pro/",
